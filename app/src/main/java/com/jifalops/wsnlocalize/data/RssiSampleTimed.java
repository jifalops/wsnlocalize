 ...
 *
 */
public class RssiSampleTimed extends RssiSample {
    ...
    public RssiSampleTimed(List<Rssi> records) {
        super(records);
        ...
        int min = (int) Stats.min(el);
        int max = (int) Stats.max(el);
    ...
    public RssiSampleTimed(String[] csv) {
        super(csv);
        System.arraycopy(csv, 7, csv, 0, 7); // shift 7 elements left 7 places
        elapsed = new Elapsed(csv);
    }

    public RssiSampleTimed(double[] a) {
        super(a);
        Arrays.shiftLeft(a, super.toArray().length - super.getNumOutputs());
        elapsed = new Elapsed((long)a[0], (int)a[1], (int)a[2], (int)a[3], a[4], a[5], a[6]);
    }
